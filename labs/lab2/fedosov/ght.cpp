...
int main(int argc, char* argv[])
{
  ...
  vector<Point2f> objectPoints(matches.size());
  vector<Point2f> scenePoints(matches.size());
  KeyPoint pointOnObject;
  KeyPoint pointOnScene;
  float distanceToCentre;
  float directionToCentre;
  float newDistanceToCentre;
  float newdDirectionToCentre;
  float delta_a;
  float delta_scale;
  for (int matchInd = 0; matchInd < matches.size(); matchInd++)
  {
      pointOnObject = keypoints1[matches[matchInd].queryIdx];
      distanceToCentre = (float)norm(pointOnObject.pt - center);
      directionToCentre = atan( (pointOnObject.pt.y - center.y) / (pointOnObject.pt.x - center.x));

      pointOnScene = keypoints2[matches[matchInd].trainIdx];
      delta_a = (pointOnScene.angle - pointOnObject.angle) * CV_PI / 180.0f;
      delta_scale = pointOnScene.size / pointOnObject.size;

      newDistanceToCentre = distanceToCentre * delta_scale;
      newdDirectionToCentre = directionToCentre + delta_a;
      
      Point2f centreOfScene(pointOnScene.pt.x + newDistanceToCentre*cos(newdDirectionToCentre),
        pointOnScene.pt.y + newDistanceToCentre*sin(newdDirectionToCentre));

       hgt.at<int>( centreOfScene.y / splitSet , centreOfScene.x / splitSet )++;

       objectPoints[matchInd] = keypoints1[matches[matchInd].queryIdx].pt;
       scenePoints[matchInd] = keypoints2[matches[matchInd].trainIdx].pt;
  }
  Point maxP;
  ...
  Mat hommat = findHomography(objectPoints,scenePoints, CV_RANSAC);
  Mat transformedImage;
  warpPerspective(image, transformedImage, hommat, scene.size());
  imshow("Homography", transformedImage);
  waitKey(0);
  return 0;
}